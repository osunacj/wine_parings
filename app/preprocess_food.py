...
def extract_ingredients(all_raw_ingredients):
    ...
    cleaned_ingredients = list(ingredient_normalizer.normalize_ingredients(list_ingredients))

    cleaned_ingredients = sorted(list({elem.strip() for elem in cleaned_ingredients if len(elem.split()) <= 3 and len(elem) > 1}))

    pd.Series( cleaned_ingredients).to_csv(BASE_PATH + '/food_reviews/ingredients.csv')

    return cleaned_ingredients


def  normalize_instructions(instructions_list, ingredients_set):
...
def main():
    ...
    normalized_instructions_token = normalize_instructions(food_dataset['steps'].to_numpy()[:500], ingredients_set)
    normalized_name_token = normalize_instructions(food_dataset['name'].to_numpy(), ingredients_set)
    normalized_description_token = normalize_instructions(food_dataset['description'].to_numpy()[:500], ingredients_set)
