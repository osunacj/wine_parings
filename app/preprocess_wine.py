...
def extract_term_frequeuncies_from_bigrams(wine_bigrams):
    ...
    print(
        f"In total found from 15 to 100: {len([elem for elem in wine_terms_sorted if elem[1] > 15 and elem[1] < 100])} ingredients"
    )
    print(
        f"In total found from 100 to 300: {len([elem for elem in wine_terms_sorted if elem[1] > 99 and elem[1] < 300])} ingredients"
    )
    print(
        f"In total found from 300 to 600: {len([elem for elem in wine_terms_sorted if elem[1] > 299 and elem[1] < 600])} ingredients"
    )
    ...
    for instructions in tqdm(reviews, total=len(reviews)):
        ...
        normalized_instructions.append(
            instruction_normalizer.normalize_instruction(
                instruction_text, ingredients_set
            )
        )
...
def main():
    ...
    all_wine_corpus = " ".join(
        str(sentence).lower() for sentence in wine_dataframe.Description.to_numpy()
    )
