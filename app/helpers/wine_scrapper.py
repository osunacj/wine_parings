...
def scrape_wine_links(base_url, min_page_number, max_page_number):
    ...
    series_wine_pages.to_csv("./app/data/wine_pages_to_mine.csv")
...
class WineInfoScraper:
    ...
    def get_reviewer_name(self, soup):
        ...
        wine_reviewer_info_list_no_blanks = [
            w for w in wine_reviewer_info_list if len(w) > 1
        ]
    ...
    def scrape_all_info(self):
        ...
        wine_info_dict.update(
            self.get_wine_info(wine_review_soup, primary_secondary="primary-info")
        )
        wine_info_dict.update(
            self.get_wine_info(wine_review_soup, primary_secondary="secondary-info")
        )

        wine_info_dict["Reviewer"] = self.get_reviewer_name(wine_review_soup)
        wine_info_dict["Reviewer Twitter Handle"] = self.get_reviewer_twitter_handle(
            wine_review_soup
        )
...
def mine_all_wine_info():
    ...
    for link in all_wine_links:
        try:
            scraper = WineInfoScraper(
                wine_page_to_mine=link
    ...
    full_wine_info_dataframe = full_wine_info_dataframe[
        [
            "Alcohol",
            "Appellation",
            "Bottle Size",
    ...
    full_wine_info_dataframe.to_csv("./app/data/all_scraped_wine_info_zinfandel.csv")
